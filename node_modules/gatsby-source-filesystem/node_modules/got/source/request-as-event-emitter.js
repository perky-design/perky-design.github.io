...
module.exports = (options, input) => {
  ...
  const setCookie = options.cookieJar
    ? util.promisify(options.cookieJar.setCookie.bind(options.cookieJar))
    : null;
  const getCookieString = options.cookieJar
    ? util.promisify(options.cookieJar.getCookieString.bind(options.cookieJar))
    : null;
    ...
    const handleResponse = async response => {
      try {
        ...
        if (options.cookieJar && rawCookies) {
          await Promise.all(
            rawCookies.map(rawCookie => setCookie(rawCookie, response.url))
          );
        }
  ...
  setImmediate(async () => {
    try {
      ...
      if (
        ...
        if (
          (uploadBodySize > 0 || options.method === "PUT") &&
      ...
      requestUrl =
        options.href ||
        new URL(options.path, urlLib.format(options)).toString();
