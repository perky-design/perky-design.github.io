...
const preNormalize = (options, defaults) => {
  ...
  if (is.nullOrUndefined(options.hooks)) {
    ...
    throw new TypeError(
      `Parameter \`hooks\` must be an object, not ${is(options.hooks)}`
    );
  }
  ...
  if (options.gotTimeout) {
    options.retry.maxRetryAfter = Math.min(
      ...[options.gotTimeout.request, options.gotTimeout.connection].filter(
        n => !is.nullOrUndefined(n)
      )
    );
  }
...
const normalize = (url, options, defaults) => {
  ...
  if (!is.string(url) && !is.object(url)) {
    throw new TypeError(
      `Parameter \`url\` must be a string or object, not ${is(url)}`
    );
  }
  ...
  if (is.nullOrUndefined(body)) {
    ...
    const isObject =
      is.object(body) && !is.buffer(body) && !is.nodeStream(body);
    if (
      !is.nodeStream(body) &&
      !is.string(body) &&
      !is.buffer(body) &&
  ...
  if (!is.function(options.retry.retries)) {
    ...
    options.retry.retries = (iteration, error) => {
      ...
      if (
        (!error || !options.retry.errorCodes.has(error.code)) &&
