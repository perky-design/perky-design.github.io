  ...
  for (var i = 0; i > -1; ) {
    ...
    if (nodeWithData) {
      referencedCodePoints =
        current & DATA_DUPLET_FLAG ? [neTree[++i], neTree[++i]] : [neTree[++i]];
...
_[MARKUP_DECLARATION_OPEN_STATE] = function markupDeclarationOpenState(cp) {
  var dashDashMatch = this._consumeSubsequentIfMatch(
      $$.DASH_DASH_STRING,
      cp,
      true
    ),
    doctypeMatch =
      !dashDashMatch &&
      this._consumeSubsequentIfMatch($$.DOCTYPE_STRING, cp, false),
    cdataMatch =
      !dashDashMatch &&
      !doctypeMatch &&
      this.allowCDATA &&
...
_[AFTER_DOCTYPE_NAME_STATE] = function afterDoctypeNameState(cp) {
  ...
  if (cp === $.GREATER_THAN_SIGN) {
    ...
    var publicMatch = this._consumeSubsequentIfMatch(
        $$.PUBLIC_STRING,
        cp,
        false
      ),
      systemMatch =
        !publicMatch &&
        this._consumeSubsequentIfMatch($$.SYSTEM_STRING, cp, false);
...
_[CDATA_SECTION_STATE] = function cdataSectionState(cp) {
  while (true) {
    if (cp === $.EOF) {
      ...
      var cdataEndMatch = this._consumeSubsequentIfMatch(
        $$.CDATA_END_STRING,
        cp,
        true
      );
