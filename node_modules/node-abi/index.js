...
function getTarget(abi, runtime) {
  ...
  throw new Error(
    "Could not detect target for abi " + abi + " and runtime " + runtime
  );
}
...
var supportedTargets = [
  ...
  { runtime: "node", target: "9.0.0", abi: "59", lts: false },
  {
    ...
    abi: "64",
    lts:
      new Date(2018, 10, 1) < new Date() && new Date() < new Date(2020, 4, 31)
  },
  { runtime: "node", target: "11.0.0", abi: "67", lts: false },
  {
    ...
    abi: "72",
    lts:
      new Date(2019, 9, 21) < new Date() && new Date() < new Date(2020, 9, 31)
  },
