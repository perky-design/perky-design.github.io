...
const encodings = (
  ...
  (process.browser ? "" : " ucs2 ucs-2 utf16le utf-16le")
).split(" ");
...
tape("append accepts arrays of BufferLists", function(t) {
  ...
  bl.append([
    Buffer.from("mnop"),
    new BufferList([Buffer.from("qrstu"), Buffer.from("vwxyz")])
  ]);
  t.equal(bl.length, 26);
  t.equal(bl.slice().toString("ascii"), "abcdefghijklmnopqrstuvwxyz");
...
tape("test readUInt16LE / readUInt16BE / readInt16LE / readInt16BE", function(
  t
) {
  const buf1 = Buffer.alloc(1);
  const buf2 = Buffer.alloc(3);
...
tape("test readUInt32LE / readUInt32BE / readInt32LE / readInt32BE", function(
  t
) {
  const buf1 = Buffer.alloc(1);
  const buf2 = Buffer.alloc(3);
...
!process.browser &&
  tape("test stream", function(t) {
    ...
    const bl = new BufferList((err, buf) => {
      ...
      bl.pipe(fs.createWriteStream("/tmp/bl_test_rnd_out.dat")).on(
        "close",
        function() {
          const rndhash = crypto
            .createHash("md5")
            .update(random)
            .digest("hex");
