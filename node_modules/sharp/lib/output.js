...
function toFile(fileOut, callback) {
  if (!fileOut || fileOut.length === 0) {
    ...
    if (this.options.input.file === fileOut) {
      const errOutputIsInput = new Error(
...
function withMetadata(options) {
  ...
  if (is.object(options)) {
    if (is.defined(options.orientation)) {
      if (
        is.integer(options.orientation) &&
        is.inRange(options.orientation, 1, 8)
      ) {
...
function toFormat(format, options) {
  const actualFormat = formats.get(
    is.object(format) && is.string(format.id) ? format.id : format
  );
  if (!actualFormat) {
    throw is.invalidParameterError(
      "format",
...
function jpeg(options) {
  if (is.object(options)) {
    ...
    if (is.defined(quantisationTable)) {
      if (
        is.integer(quantisationTable) &&
        is.inRange(quantisationTable, 0, 8)
      ) {
  ...
  if (is.object(options)) {
    ...
    if (is.defined(options.palette)) {
      ...
      if (this.options.pngPalette) {
        if (is.defined(options.quality)) {
          if (
            is.integer(options.quality) &&
            is.inRange(options.quality, 0, 100)
          ) {
...
function webp(options) {
  ...
  if (is.object(options) && is.defined(options.alphaQuality)) {
    if (
      is.integer(options.alphaQuality) &&
      is.inRange(options.alphaQuality, 0, 100)
    ) {
  ...
  if (is.object(options) && is.defined(options.reductionEffort)) {
    if (
      is.integer(options.reductionEffort) &&
      is.inRange(options.reductionEffort, 0, 6)
    ) {
  ...
  if (is.object(options)) {
    ...
    if (is.defined(options.compression)) {
      if (
        ...
        throw is.invalidParameterError(
          "compression",
          "one of: lzw, deflate, jpeg, ccittfax4, none",
          options.compression
        );
      }
    }
...
function heif(options) {
  ...
  if (is.object(options)) {
    ...
    if (is.defined(options.compression)) {
      if (
        is.string(options.compression) &&
        is.inArray(options.compression, ["hevc", "avc", "jpeg", "av1"])
      ) {
        this.options.heifCompression = options.compression;
      } else {
        throw is.invalidParameterError(
          "compression",
          "one of: hevc, avc, jpeg, av1",
          options.compression
        );
      }
    }
...
function tile(options) {
  if (is.object(options)) {
    ...
    if (is.defined(options.container)) {
      if (
        is.string(options.container) &&
        is.inArray(options.container, ["fs", "zip"])
      ) {
    ...
    if (is.defined(options.layout)) {
      if (
        ...
        is.inArray(options.layout, ["dz", "google", "iiif", "zoomify"])
      ) {
        this.options.tileLayout = options.layout;
      } else {
        throw is.invalidParameterError(
          "layout",
          "one of: dz, google, iiif, zoomify",
          options.layout
        );
      }
    }
    ...
    if (is.defined(options.skipBlanks)) {
      if (
        is.integer(options.skipBlanks) &&
        is.inRange(options.skipBlanks, -1, 65535)
      ) {
