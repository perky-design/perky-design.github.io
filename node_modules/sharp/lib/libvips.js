...
const minimumLibvipsVersion = semver.coerce(minimumLibvipsVersionLabelled)
  .version;
...
const globalLibvipsVersion = function() {
  if (process.platform !== "win32") {
    const globalLibvipsVersion =
      spawnSync(
        `PKG_CONFIG_PATH="${pkgConfigPath()}" pkg-config --modversion vips-cpp`,
        spawnSyncOptions
      ).stdout || "";
...
const hasVendoredLibvips = function() {
  ...
  if (vendorVersionId && vendorVersionId !== minimumLibvipsVersion) {
    throw new Error(
      `Found vendored libvips v${vendorVersionId} but require v${minimumLibvipsVersion}. Please remove the 'node_modules/sharp/vendor' directory and run 'npm install'.`
    );
  }
  ...
  if (vendorPlatformId) {
    ...
    if (currentPlatformId === vendorPlatformId) {
      ...
      throw new Error(
        `'${vendorPlatformId}' binaries cannot be used on the '${currentPlatformId}' platform. Please remove the 'node_modules/sharp/vendor' directory and run 'npm install'.`
      );
    }
  } else {
    return false;
