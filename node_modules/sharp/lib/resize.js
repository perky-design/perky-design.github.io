...
function resize(width, height, options) {
  ...
  if (is.object(options)) {
    ...
    if (is.defined(options.position)) {
      ...
      if (
        is.integer(pos) &&
        (is.inRange(pos, 0, 8) || is.inRange(pos, 16, 17))
      ) {
...
function extend(extend) {
  if (is.integer(extend) && extend > 0) {
    ...
    is.object(extend) &&
    is.integer(extend.top) &&
    extend.top >= 0 &&
    is.integer(extend.bottom) &&
    extend.bottom >= 0 &&
    is.integer(extend.left) &&
    extend.left >= 0 &&
    is.integer(extend.right) &&
    extend.right >= 0
  ) {
...
function extract(options) {
  const suffix =
    this.options.width === -1 && this.options.height === -1 ? "Pre" : "Post";
