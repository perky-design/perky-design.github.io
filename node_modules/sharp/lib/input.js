...
function _inputOptionsFromObject(obj) {
  ...
  return [raw, density, limitInputPixels, sequentialRead, failOnError].some(
    is.defined
  )
  ...
  if (is.object(inputOptions)) {
    ...
    if (is.defined(inputOptions.limitInputPixels)) {
      if (is.bool(inputOptions.limitInputPixels)) {
        ...
        inputOptions.limitInputPixels >= 0
      ) {
        inputDescriptor.limitInputPixels = inputOptions.limitInputPixels;
      } else {
        throw is.invalidParameterError(
    ...
    if (is.defined(inputOptions.pages)) {
      if (
        is.integer(inputOptions.pages) &&
        is.inRange(inputOptions.pages, -1, 100000)
      ) {
    ...
    if (is.defined(inputOptions.page)) {
      if (
        is.integer(inputOptions.page) &&
        is.inRange(inputOptions.page, 0, 100000)
      ) {
    ...
    if (is.defined(inputOptions.create)) {
      if (
        ...
        is.inRange(inputOptions.create.channels, 3, 4) &&
        ...
        throw new Error(
          "Expected width, height, channels and background to create a new input image"
        );
      }
    }
