...
function getTargets(targetAttr) {
  ...
  return targetAttr
    ? ("" + targetAttr).split(/\s+/).map(function(target) {
        return "#" + target;
      })
    : undefined;
}
...
var evaluateExecutableContent = function(context, _ev, meta, body) {
  ...
  var fnBody = "\n    " + scope + "\n    " + body + "\n  ";
  var fn = new (Function.bind.apply(
    Function,
    __spread([void 0], args, [fnBody])
  ))();
...
function mapAction(element) {
  ...
  switch (element.name) {
    ...
    case "send": {
      var _b = element.attributes,
        event_1 = _b.event,
        eventexpr_1 = _b.eventexpr,
        target = _b.target;
    ...
    case "if": {
      ...
      try {
        for (
          var _c = __values(element.elements), _d = _c.next();
          !_d.done;
          _d = _c.next()
        ) {
          var el = _d.value;
...
function mapActions(elements) {
  ...
  try {
    for (
      var elements_1 = __values(elements), elements_1_1 = elements_1.next();
      !elements_1_1.done;
...
function toConfig(nodeJson, id, options) {
  ...
  if (nodeJson.elements) {
    ...
    var invokeElements = nodeJson.elements.filter(function(element) {
      return element.name === "invoke";
    });
    ...
    var states = indexedRecord(stateElements, function(item) {
      return "" + item.attributes.id;
    });
    var initialElement = !initial
      ? nodeJson.elements.find(function(element) {
    ...
    var on = transitionElements.map(function(value) {
      ...
      return __assign(
        __assign(
          __assign(
            { event: event, target: getTargets(targets) },
    ...
    var invoke = invokeElements.map(function(element) {
      ...
      var content = element.elements.find(function(el) {
        return el.name === "content";
      });
      ...
      __assign(
        __assign(
          __assign(
            ...
            transitionElements.length ? { on: on } : undefined
          ),
          onEntry ? { onEntry: onEntry } : undefined
        ),
        onExit ? { onExit: onExit } : undefined
      ),
      invoke.length ? { invoke: invoke } : undefined
    );
  }
  return { id: id };
}
function scxmlToMachine(scxmlJson, options) {
  var machineElement = scxmlJson.elements.find(function(element) {
  ...
  var extState = dataModelEl
    ...
    : undefined;
  return index_1.Machine(
    __assign(__assign({}, toConfig(machineElement, "(machine)", options)), {
      context: extState,
