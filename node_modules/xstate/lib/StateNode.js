  ...
  var hasNonLastUnguardedTarget = transitions
    ...
    .some(function(transition) {
      return (
        !("cond" in transition) &&
        !("in" in transition) &&
        (utils_1.isString(transition.target) ||
          utils_1.isMachine(transition.target))
      );
    });
  var eventText =
...
var StateNode = /** @class */ (function() {
    ...
    this.states = this.config.states
      ? utils_1.mapValues(this.config.states, function(stateConfig, key) {
          ...
          Object.assign(
            _this.idMap,
            __assign(
              ((_a = {}), (_a[stateNode.id] = stateNode), _a),
              stateNode.idMap
            )
          );
          return stateNode;
    ...
    this.invoke = utils_1
      ...
      .map(function(invokeConfig, i) {
        ...
        if (utils_1.isMachine(invokeConfig)) {
          ...
          return __assign(
            __assign({ type: actionTypes.invoke, id: invokeSrc }, invokeConfig),
            { src: invokeSrc }
          );
        } else {
  ...
  StateNode.prototype.getDelayedTransitions = function() {
    ...
    var mutateEntryExit = function(delay, i) {
      var delayRef = utils_1.isFunction(delay)
        ? _this.id + ":delay[" + i + "]"
        : delay;
    ...
    var delayedTransitions = utils_1.isArray(afterConfig)
      ...
      : utils_1.flatten(
          utils_1.keys(afterConfig).map(function(delay, i) {
  ...
  StateNode.prototype.getStateNodes = function(state) {
    ...
    if (utils_1.isString(stateValue)) {
      ...
      return initialStateValue !== undefined
        ? this.getStateNodes(
            ((_a = {}), (_a[stateValue] = initialStateValue), _a)
          )
  ...
  StateNode.prototype.resolveState = function(state) {
    var configuration = Array.from(
      stateUtils_1.getConfiguration([], this.getStateNodes(state.value))
    );
  ...
  StateNode.prototype.transitionCompoundNode = function(
    stateValue,
    state,
    _event
  ) {
  ...
  StateNode.prototype.transitionParallelNode = function(
    stateValue,
    state,
    _event
  ) {
    ...
    try {
      for (
        var _b = __values(utils_1.keys(stateValue)), _c = _b.next();
        !_c.done;
        _c = _b.next()
      ) {
    ...
    var stateTransitions = utils_1.keys(transitionMap).map(function(key) {
      return transitionMap[key];
    });
    var enabledTransitions = utils_1.flatten(
    ...
    var entryNodes = utils_1.flatten(
      stateTransitions.map(function(t) {
        return t.entrySet;
      })
    );
    var configuration = utils_1.flatten(
      utils_1.keys(transitionMap).map(function(key) {
  ...
  StateNode.prototype.next = function(state, _event) {
        ...
        var isInState = stateIn
          ? utils_1.isString(stateIn) && isStateId(stateIn)
            ? // Check if in state by ID
              state.matches(
                utils_1.toStateValue(
                  this.getStateNodeById(stateIn).path,
                  this.delimiter
                )
              )
            : // Check if in state by relative grandparent
              utils_2.matchesState(
                utils_1.toStateValue(stateIn, this.delimiter),
                utils_1.path(this.path.slice(0, -2))(state.value)
              )
          : true;
    ...
    var allNextStateNodes = utils_1.flatten(
  ...
  StateNode.prototype.getActions = function(
    transition,
    currentContext,
    _event,
    prevState
    ...
    var e_4, _a, e_5, _b;
    var prevConfig = stateUtils_1.getConfiguration(
      [],
    ...
    try {
      for (
        ...
        if (
          !stateUtils_1.has(resolvedConfig, sn) ||
          stateUtils_1.has(transition.exitSet, sn.parent)
        ) {
    ...
    var doneEvents = utils_1.flatten(
      transition.entrySet.map(function(sn) {
        var events = [];
        if (sn.type !== "final") {
          return events;
        }
        ...
        events.push(
          actions_1.done(sn.id, sn.data), // TODO: deprecate - final states should not emit done events for their own state.
          actions_1.done(
            parent.id,
            sn.data
              ? utils_1.mapContext(sn.data, currentContext, _event)
              : undefined
          )
        );
        ...
        if (grandparent.type === "parallel") {
          if (
            stateUtils_1.getChildren(grandparent).every(function(parentNode) {
              return stateUtils_1.isInFinalState(
    ...
    var _c = __read(
      ...
      entryActions = _c[0],
      exitActions = _c[1];
    var actions = actions_1.toActionObjects(
  ...
  StateNode.prototype.resolveRaisedTransition = function(
    state,
    _event,
    originalEvent
  ) {
  ...
  StateNode.prototype.resolveTransition = function(
    stateTransition,
    currentState,
    _event,
    context
    ...
    var _b = __read(
      ...
      updatedContext = _b[1];
    var _c = __read(
        utils_1.partition(resolvedActions, function(action) {
          return (
            action.type === actionTypes.raise ||
            (action.type === actionTypes.send &&
              action.to === types_1.SpecialTargets.Internal)
          );
        }),
        2
      ),
  ...
  StateNode.prototype.resolve = function(stateValue) {
    ...
    switch (this.type) {
      case "parallel":
        return utils_1.mapValues(this.initialStateValue, function(
          subStateValue,
          subStateKey
        ) {
      ...
      case "compound":
        if (utils_1.isString(stateValue)) {
          ...
          if (
            ...
            return (
              (_a = {}), (_a[stateValue] = subStateNode.initialStateValue), _a
            );
          }
          return stateValue;
        }
        ...
        return utils_1.mapValues(stateValue, function(
          subStateValue,
          subStateKey
        ) {
          return subStateValue
  ...
  StateNode.prototype.getResolvedPath = function(stateIdentifier) {
    if (isStateId(stateIdentifier)) {
      var stateNode = this.machine.idMap[
  ...
  Object.defineProperty(StateNode.prototype, "initialStateValue", {
    get: function() {
      ...
      if (this.type === "parallel") {
        initialStateValue = utils_1.mapFilterValues(
          this.states,
          function(state) {
   ...
   */
  StateNode.prototype.getRelativeStateNodes = function(
    relativeStateId,
    historyValue,
    resolve
  ) {
  ...
  StateNode.prototype.historyValue = function(relativeStateValue) {
    ...
    return {
      ...
      states: utils_1.mapFilterValues(
        this.states,
  ...
  StateNode.prototype.resolveHistory = function(historyValue) {
    ...
    var subHistoryValue = utils_1.nestedPath(
      parent.path,
      "states"
    ...
    return utils_1.flatten(
      utils_1.toStatePaths(subHistoryValue).map(function(subStatePath) {
  ...
  Object.defineProperty(StateNode.prototype, "stateIds", {
    ...
    get: function() {
      var _this = this;
      var childStateIds = utils_1.flatten(
        utils_1.keys(this.states).map(function(stateKey) {
  ...
  Object.defineProperty(StateNode.prototype, "events", {
    ...
    get: function() {
      ...
      if (states) {
        try {
          for (
            var _c = __values(utils_1.keys(states)), _d = _c.next();
            !_d.done;
            _d = _c.next()
          ) {
    ...
    var internal =
      "internal" in transitionConfig
        ...
        ? normalizedTarget.some(function(_target) {
            return utils_1.isString(_target) && _target[0] === _this.delimiter;
          })
        : true;
    var guards = this.machine.options.guards;
    var target = this.resolveTarget(normalizedTarget);
    var transition = __assign(__assign({}, transitionConfig), {
      actions: actions_1.toActionObjects(
        utils_1.toArray(transitionConfig.actions)
      ),
      cond: utils_1.toGuard(transitionConfig.cond, guards),
      target: target,
      source: this,
      internal: internal,
  ...
  StateNode.prototype.formatTransitions = function() {
    ...
    if (!this.config.on) {
      ...
      onConfig = utils_1.flatten(
        utils_1
          .keys(strictOnConfigs_1)
          .map(function(key) {
            var arrayified = utils_1.toTransitionConfigArray(
              key,
              strictOnConfigs_1[key]
            );
            if (!environment_1.IS_PRODUCTION) {
    ...
    var doneConfig = this.config.onDone
      ? utils_1.toTransitionConfigArray(
          String(actions_1.done(this.id)),
          this.config.onDone
        )
      : [];
    if (!environment_1.IS_PRODUCTION) {
    ...
    var invokeConfig = utils_1.flatten(
      this.invoke.map(function(invokeDef) {
        ...
        if (invokeDef.onDone) {
          settleTransitions.push.apply(
            ...
            __spread(
              utils_1.toTransitionConfigArray(
                String(actions_1.doneInvoke(invokeDef.id)),
                invokeDef.onDone
              )
            )
          );
        ...
        if (invokeDef.onError) {
          settleTransitions.push.apply(
            ...
            __spread(
              utils_1.toTransitionConfigArray(
                String(actions_1.error(invokeDef.id)),
                invokeDef.onError
              )
            )
          );
    ...
    var formattedTransitions = utils_1.flatten(
      __spread(doneConfig, invokeConfig, onConfig).map(function(
        transitionConfig
      ) {
