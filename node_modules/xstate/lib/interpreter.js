...
var Interpreter = /** @class */ (function() {
  ...
  function Interpreter(machine, options) {
    ...
    this.send = function(event, payload) {
      ...
      if (
        _this._status === InterpreterStatus.NotStarted &&
    ...
    this.sendTo = function(event, to) {
      var isParent =
        _this.parent &&
        (to === types_1.SpecialTargets.Parent || _this.parent.id === to);
      var target = isParent
        ? _this.parent
        : Actor_1.isActor(to)
        ? to
    ...
    var resolvedOptions = __assign(
      __assign({}, Interpreter.defaultOptions),
      options
    );
  ...
  Interpreter.prototype.update = function(state, _event) {
    ...
    var isDone = stateUtils_1.isInFinalState(
      state.configuration || [],
      this.machine
    );
  ...
  Interpreter.prototype.subscribe = function(
        ...
        : withServiceScope(this, function() {
            return State_1.isState(initialState)
              ...
              : _this.machine.resolveState(
                  State_1.State.from(initialState, _this.machine.context)
                );
          });
  ...
  Interpreter.prototype.stop = function() {
    ...
    try {
      ...
      for (
        var _p = __values(utils_1.keys(this.delayedEventsMap)), _q = _p.next();
        !_q.done;
        _q = _p.next()
      ) {
        var key = _q.value;
  ...
  Interpreter.prototype.exec = function(action, state, actionFunctionMap) {
    ...
    var actionOrExec =
      action.exec ||
      actions_1.getActionFunction(action.type, actionFunctionMap);
  ...
  Interpreter.prototype.spawn = function(entity, name, options) {
    if (utils_1.isPromiseLike(entity)) {
      ...
      return this.spawnMachine(
        entity,
        __assign(__assign({}, options), { id: name })
      );
    } else {
      throw new Error(
  ...
  Interpreter.prototype.spawnMachine = function(machine, options) {
    ...
    var childService = new Interpreter(
      machine,
    ...
    childService
      .onDone(function(doneEvent) {
        ...
        _this.send(
          utils_1.toSCXMLEvent(doneEvent, { origin: childService.id })
        );
      })
      .start();
    return actor;
    ...
    promise.then(
      function(response) {
        if (!canceled) {
          ...
          _this.send(
            utils_1.toSCXMLEvent(actions_1.doneInvoke(id, response), {
              origin: id
            })
          );
        }
  ...
  Interpreter.prototype.spawnObservable = function(source, id) {
    ...
    var subscription = source.subscribe(
      ...
      function(err) {
        _this.removeChild(id);
        _this.send(
          utils_1.toSCXMLEvent(actions_1.error(id, err), { origin: id })
        );
      },
      function() {
        _this.removeChild(id);
        _this.send(
          utils_1.toSCXMLEvent(actions_1.doneInvoke(id), { origin: id })
        );
      }
    );
    var actor = {
...
var resolveSpawnOptions = function(nameOrOptions) {
  ...
  return __assign(
    __assign(__assign({}, DEFAULT_SPAWN_OPTIONS), { name: utils_1.uniqueId() }),
    nameOrOptions
  );
};
    ...
    if (!environment_1.IS_PRODUCTION) {
      utils_1.warn(
        ...
        'Attempted to spawn an Actor (ID: "' +
          (utils_1.isMachine(entity) ? entity.id : "undefined") +
