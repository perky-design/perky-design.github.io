...
const invalidDestinationDirMessage = dir =>
  `[gatsby-remark-copy-linked-files You have supplied an invalid destination directory. The destination directory must be a child but was: ${dir}`; // dest must be a child
...
module.exports = (
  ...
  visit(markdownAST, [`html`, `jsx`], node => {
    ...
    extractUrlAttributeAndElement(
      ...
      `src`
    ).forEach(processUrl); // Handle video poster.
    ...
    extractUrlAttributeAndElement(
      ...
      `src`
    ).forEach(processUrl); // Handle flash embed tags.
