...
module.exports = (
  ...
  visit(markdownAST, `code`, node => {
    ...
    const useCommandLine = [`bash`, `shell`].includes(languageName) && (prompt.global || outputLines && outputLines.length > 0 || promptUserLocal || promptHostLocal); // prettier-ignore
    ...
    node.value =
      ...
      `${
        useCommandLine
          ? commandLine(node.value, outputLines, promptUser, promptHost)
          : ``
      }` +
      `${highlightedCode}` +
      `</code>` +
