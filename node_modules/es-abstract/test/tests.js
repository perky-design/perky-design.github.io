...
var es2015 = function ES2015(ES, ops, expectedMissing, skips) {
	...
	test("GetIterator", function(t) {
		...
		t.test("Symbol.iterator", { skip: !v.hasSymbols }, function(st) {
			...
			testIterator(st, ES.GetIterator(m), [
				[1, "a"],
				[2, "b"]
			]);
	...
	test("CreateDataProperty", function(t) {
		...
		forEach(v.propertyKeys, function(propertyKey) {
			...
			var secondStatus = ES.CreateDataProperty(
				obj,
				propertyKey,
				secondSentinel
			);
			...
			t.equal(
				obj[propertyKey],
				secondSentinel,
				debug(secondSentinel) +
					' is installed on "' +
					debug(propertyKey) +
					'" on the object'
			);

			t.test("with defineProperty", { skip: !defineProperty.oDP }, function(
				st
			) {
	...
	test("ValidateAndApplyPropertyDescriptor", function(t) {
		...
		t.test("current is undefined", function(st) {
			...
			st.test("accessor descriptor", { skip: !defineProperty.oDP }, function(
				s2t
			) {
				var count = 0;
				var accessor = v.accessorDescriptor();
		...
		t.test(
			...
			function(st) {
				...
				var shouldBeAccessor = getOwnPropertyDescriptor(
					startsWithData,
					"property key"
				);
				...
				var shouldBeData = getOwnPropertyDescriptor(
					startsWithAccessor,
					"property key"
				);
				st.deepEqual(
	...
	test("SetFunctionName", function(t) {
		...
		t.test("has an own name property", { skip: !functionsHaveNames }, function(
			st
		) {
			st["throws"](
				function() {
...
var es2016 = function ES2016(ES, ops, expectedMissing, skips) {
	...
	test("OrdinarySetPrototypeOf", { skip: !$getProto || !$setProto }, function(
		t
	) {
		var a = [];
		var proto = {};
