...
- [Get interleaved output](#all) from `stdout` and `stderr` similar to what is printed on the terminal. [_(Async only)_](#execasyncfile-arguments-options)
...
const run = async () => {
  const results = await execa("curl", [
    "-sSL",
