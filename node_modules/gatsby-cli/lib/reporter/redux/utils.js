...
const getGlobalStatus = (id, status) => {
  const { logs } = (0, _index.getStore)().getState();
  ...
  return currentActivities.reduce((generatedStatus, activityId) => {
    ...
    if (
      activityStatus === _constants.ActivityStatuses.InProgress ||
      activityStatus === _constants.ActivityStatuses.NotStarted
    ) {
      return _constants.ActivityStatuses.InProgress;
    } else if (
      activityStatus === _constants.ActivityStatuses.Failed &&
      generatedStatus !== _constants.ActivityStatuses.InProgress
    ) {
      return _constants.ActivityStatuses.Failed;
    } else if (
      activityStatus === _constants.ActivityStatuses.Interrupted &&
      generatedStatus !== _constants.ActivityStatuses.InProgress
    ) {
