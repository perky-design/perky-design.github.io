...
const sanitizeStructuredStackTrace = stack => {
  ...
  stack = stack.filter(callSite => {
    ...
    if (
      (0, _gatsbyCoreUtils.isNodeInternalModulePath)(callSite.getFileName())
    ) {
      return false;
    }
...
function getErrorFormatter() {
  ...
  prettyError.skipPackage(
    `regenerator-runtime`,
    `graphql`,
    `core-js` // `static-site-generator-webpack-plugin`,
    // `tapable`, // webpack
...
async function createErrorFromString(errorStr = ``, sourceMapFile) {
  ...
  try {
    if (sourceMapFile) {
      return await (0, _prepareStackTrace.prepareStackTrace)(
        error,
        sourceMapFile
      );
    }
