...
const sanitizeAction = action => {
  ...
  if (`text` in copiedAction.payload && copiedAction.payload.text) {
    copiedAction.payload.text = (0, _stripAnsi.default)(
      copiedAction.payload.text
    );
  }

  if (`statusText` in copiedAction.payload && copiedAction.payload.statusText) {
    copiedAction.payload.statusText = (0, _stripAnsi.default)(
...
const initializeIPCLogger = () => {
  (0, _index.onLogAction)(action => {
    ...
    if (sanitizedAction.type === _constants.Actions.Log) {
      ...
      if (
        [_constants.LogLevels.Debug].includes(sanitizedAction.payload.level)
      ) {
        return;
      } // Override Success and Log types to Info over IPC

      if (
        [_constants.LogLevels.Success, _constants.LogLevels.Log].includes(
          sanitizedAction.payload.level
        )
      ) {
