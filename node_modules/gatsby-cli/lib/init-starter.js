...
const shouldUseYarn = async () => {
  try {
    ...
    if (!packageManager) {
      ...
      if ((0, _isTty.isTTY)()) {
        packageManager =
          (await (0, _packageManager.promptPackageManager)()) || `yarn`;
      } else {
        packageManager = `yarn`;
      }
    }
...
const maybeCreateGitIgnore = async rootPath => {
  ...
  await _fsExtra.default.writeFile(
    _path.default.join(rootPath, `.gitignore`),
    `.cache\nnode_modules\npublic\n`
  );
...
const createInitialGitCommit = async (rootPath, starterUrl) => {
  ...
  try {
    (0, _child_process.execSync)(
      `git commit -m "Initial commit from gatsby: (${starterUrl})"`,
      {
        cwd: rootPath
      }
    );
...
const copy = async (starterPath, rootPath) => {
  ...
  _reporter.default.info(
    `Creating new site from local starter: ${starterPath}`
  );
...
async function initStarter(starter, root) {
  ...
  if (urlObject.protocol && urlObject.host) {
    ...
    const isStarterAUrl =
      starter &&
      !_url.default.parse(starter).hostname &&
      !_url.default.parse(starter).protocol;
