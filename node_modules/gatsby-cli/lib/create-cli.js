...
const handlerP = fn => (...args) => {
  Promise.resolve(fn(...args)).then(
    () => process.exit(0),
    err => _reporter.default.panic(err)
  );
};
...
function buildLocalCommands(cli, isLocalSite) {
  ...
  const useYarn = (0, _fsExistsCached.sync)(
    _path.default.join(directory, `yarn.lock`)
  );
  ...
  function resolveLocalCommand(command) {
    if (!isLocalSite) {
      ...
      return _reporter.default.panic(
    ...
    try {
      const cmdPath =
        _resolveCwd.default.silent(`gatsby/dist/commands/${command}`) || // Old location of commands
...
function isLocalGatsbySite() {
  ...
  try {
    const { dependencies, devDependencies } = require(_path.default.resolve(
      `./package.json`
    ));
