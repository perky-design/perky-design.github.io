...
const globalConfigPath =
  _xdgBasedir.default.config || _tmp.default.fileSync().name;
...
const getDataFilePath = (siteDir, serviceName) =>
  _path.default.join(siteDir, `${serviceName}${DATA_FILE_EXTENSION}`);
...
const createServiceLock = async (programPath, serviceName, content) => {
  ...
  try {
    ...
    const unlock = await _properLockfile.default.lock(
      serviceDataFile,
      lockfileOptions
    );
    return unlock;
...
const getService = async (programPath, serviceName) => {
  ...
  try {
    if (await _properLockfile.default.check(serviceDataFile, lockfileOptions)) {
      return JSON.parse(
        await _fsExtra.default
          .readFile(serviceDataFile, `utf8`)
          .catch(() => `null`)
      );
    }
...
const getServices = async programPath => {
  ...
  const siteDir = getSiteDir(programPath);
  const serviceNames = (await _fsExtra.default.readdir(siteDir))
