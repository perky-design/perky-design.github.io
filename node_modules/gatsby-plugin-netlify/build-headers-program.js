...
function validHeaders(headers, reporter) {
  ...
  return _lodash.default.every(
    headers,
    (headersList, path) =>
      _lodash.default.isArray(headersList) &&
      _lodash.default.every(headersList, header => {
...
function preloadHeadersByPage({ pages, manifest, pathPrefix, publicFolder }) {
  ...
  pages.forEach(page => {
    const scripts = _lodash.default.flatMap(_constants.COMMON_BUNDLES, file =>
...
function transformLink(manifest, publicFolder, pathPrefix) {
  return header =>
    header.replace(_constants.LINK_REGEX, (__, prefix, file, suffix) => {
...
function stringifyHeaders(headers) {
  return _lodash.default.reduce(
    ...
    (text, headerList, path) => {
      const headersString = _lodash.default.reduce(
        headerList,
...
const mapUserLinkHeaders = ({
  ...
  _lodash.default.mapValues(headers, headerList =>
    _lodash.default.map(
      headerList,
...
const mapUserLinkAllPageHeaders = (
  ...
  const headersList = _lodash.default.map(
    allPageHeaders,
    transformLink(manifest, publicFolder, pathPrefix)
  );
