...
function handleFirst(siftArgs, nodes) {
  ...
  const index = _lodash.default.isEmpty(siftArgs)
    ? 0
    : nodes.findIndex(
        sift({
          $and: siftArgs
...
function handleMany(siftArgs, nodes) {
  let result = _lodash.default.isEmpty(siftArgs)
    ? nodes
    : nodes.filter(
        sift({
          $and: siftArgs
...
const sortNodes = (nodes, sort, resolvedFields, stats) => {
  ...
  if (
    !sort ||
    !((nodes === null || nodes === void 0 ? void 0 : nodes.length) > 1)
  ) {
    return nodes;
  ...
  const sortFns = sortFields.map(field => v =>
    (0, _getValueAt.getValueAt)(v, field)
  );
